...
Func TogglePauseImpl($Source)
   ...
   Local $BlockInputPausePrev
	...
	If $TPaused and $Runstate = True Then
		TrayTip("[ " & $sCurrProfile & " ] " & $sBotTitle, "", 1);MMHK add profile name to tray tip
		TrayTip("[ " & $sCurrProfile & " ] " & $sBotTitle, "was Paused!", 1, $TIP_ICONEXCLAMATION);MMHK add profile name to tray tip
	...
	ElseIf $TPaused = False And $Runstate = True Then
		TrayTip("[ " & $sCurrProfile & " ] " & $sBotTitle, "", 1);MMHK add profile name to tray tip
		TrayTip("[ " & $sCurrProfile & " ] " & $sBotTitle, "was Resumed.", 1, $TIP_ICONASTERISK);MMHK add profile name to tray tip
