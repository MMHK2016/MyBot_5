...
Func DonateCC($Check = False)
	...
	Local $Scroll, $offColors[3][3] = [[0xc9e871, 0, -4], [0x010101, 0, 10], [0x000000, 0, 25]] ; MMHK ; $offColors[3][3] = [[0x000000, 0, -2], [0x262926, 0, 1], [0xF8FCF0, 0, 11]]
	While $bDonate
		checkAttackDisable($iTaBChkIdle) ; Early Take-A-Break detection

		If _Sleep($iDelayDonateCC2) Then ExitLoop
		$DonatePixel = _MultiPixelSearch(222, $y, 223, 649 + $bottomOffsetY, 1, 1, Hex(0xFFFFFF, 6), $offColors, 20) ; MMHK
...
;~ 			EndIf
			...
			If $bDonateTroop Or $bDonateSpell Then
				If $ichkExtraAlphabets = 1 Then
					...
					Local $ClanString = ""
					$ClanString = getChatString(30, $DonatePixel[1] - 28, "coc-latin-cyr") ; MMHK
					If $ClanString = "" Then
				...
				Else
					...
					Local $ClanString = ""
					$ClanString = getChatString(30, $DonatePixel[1] - 28, "coc-latinA") ; MMHK
					If $ClanString = "" Then
	    ...
	    $y = 90 ; MMHK
		$Scroll = _PixelSearch(293, 8 + $y, 295, 23 + $y, Hex(0xFFFFFF, 6), 20) ; MMHK
...
Func DonateWindow($Open = True)
	...
	If _ColorCheck(_GetPixelColor($DonatePixel[0], $DonatePixel[1], True), Hex(0xFFFFFF, 6), 5) Then ; MMHK
		Click($DonatePixel[0], $DonatePixel[1], 1, 0, "#0174") ; MMHK
	Else
	...
	Local $aDonWinOffColors[2][3] = [[0xFFFFFF, 0, 2], [0xc7c5bc, 0, 209]] ; MMHK
	Local $aDonationWindow = _MultiPixelSearch(409, 0, 410, $DEFAULT_HEIGHT, 1, 1, Hex(0xFFFFFF, 6), $aDonWinOffColors, 10) ; MMHK
...
Func DonateWindowCap(ByRef $bSkipDonTroops, ByRef $bSkipDonSpells)
	...
	If $bSkipDonTroops = False Then
		Local $sReadCCTroopsCap = getCastleDonateCap(427, $DonationWindowY + 15) ; MMHK ; use OCR to get donated/total capacity
	...
	If $bSkipDonSpells = False Then
		Local $sReadCCSpellsCap = getCastleDonateCap(420, $DonationWindowY + 220) ; MMHK ; use OCR to get donated/total capacity
...
Func RemainingCCcapacity()
	...
	$aCapTroops = getOcrSpaceCastleDonate(52, $DonatePixel[1] - 5) ; MMHK ; when the request is troops+spell
	$aCapSpells = getOcrSpaceCastleDonate(159, $DonatePixel[1] - 5) ; MMHK ; when the request is troops+spell
	...
	If Not (StringInStr($aCapTroops, "#") Or StringInStr($aCapSpells, "#")) Then; verify if the string is valid or it is just a number from request without spell
		...
		$aCapTroops = getOcrSpaceCastleDonate(85, $DonatePixel[1] - 5) ; MMHK 139 ; when the Request don't have Spell
